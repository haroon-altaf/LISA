...
class IsmReport:
    ...
    def _rankings(cls, overview: str, rank_by: str) -> pd.DataFrame | None:
        ...
        df = pd.DataFrame(
            {"Sectors": sectors, "PMI Rankings": [0] * len(sectors), f"{col_name}": [0] * len(sectors)}
        ).set_index("Sectors")
        ...
        for index, item in enumerate(contract):
            df.loc[item, "PMI Rankings"] = index - len(contract)
    ...
    def _prep_rankings_table(self) -> pd.DataFrame:
        if isinstance(self, ManufacturingPmi):
            data_row = (
                pd.Series({"Year": int(self.year), "Month": int(self.month)})
                .to_frame()
                .T.join(self.sector_ranking["PMI Rankings"].to_frame().T.reset_index(drop=True))
                .join(
                    self.sector_ranking["New Orders Rankings"].to_frame().T.reset_index(drop=True),
                    rsuffix=" - New Orders",
                )
            )
            ...
            data_row = (
                ...
                .T.join(self.sector_ranking["PMI Rankings"].to_frame().T.reset_index(drop=True))
                .join(
                    self.sector_ranking["Business Activity Rankings"].to_frame().T.reset_index(drop=True),
                    rsuffix=" - Business Activity",
                )
            )
