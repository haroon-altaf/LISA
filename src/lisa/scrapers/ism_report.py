...
class IsmReport:
    ...
    def _parse_html(cls, html_content: str) -> dict[str, Tag | ResultSet | None]:
        ...
        for section_name, navigation_steps in navigation_scheme.items():
            ...
            if type(section_content) not in [Tag, ResultSet]:
    ...
    def _transform_sections(
        cls, html_sections: dict[str, Tag | ResultSet | None]
    ) -> dict[str, str | list[pd.DataFrame] | None]:
        """
                ...
                if key == "full_pmi_table":
                    ...
                    for df in df_list:
                        try:
                            ...
                            logger.exception(
                                f"Error converting first column to datetime for section: {key}\n{df.head()}"
                            )
                            transformed_sections[key] = None
        ...
        if transformed_sections["overview"] and transformed_sections[rank_by]:
            transformed_sections["sector_ranking"] = cls._rankings(
                transformed_sections["overview"], transformed_sections[rank_by]
            )
        else:
    ...
    @classmethod
    def _rankings(cls, overview: str, rank_by: str) -> dict | None:
        """
        ...
        if grow is None or contract is None:
            return None
        grow_dict = {item: len(grow) - index for index, item in enumerate(grow)}
        contract_dict = {item: index - len(contract) for index, item in enumerate(contract)}
        rankings_dict = dict.fromkeys(sectors, 0) | grow_dict | contract_dict

        # Rank sectors (based on New Orders or Business Activity)
        grow2, contract2 = sectors_from_text(rank_by)
        if grow2 is None or contract2 is None:
            return None
        grow_dict2 = {item: len(grow2) - index for index, item in enumerate(grow2)}
        contract_dict2 = {item: index - len(contract2) for index, item in enumerate(contract2)}
        rankings_dict2 = dict.fromkeys(sectors, 0) | grow_dict2 | contract_dict2
        rankings_dict2 = {f"{k}2": v for k, v in rankings_dict2.items()}

        rankings = rankings_dict | rankings_dict2

        return rankings

    @classmethod
    def _respondents_say(cls, comments: str) -> dict | None:
        """
        ...
        txt = comments.split("\n")[1:-1]
        pattern = rf"[\[\(]\s*({'|'.join(sectors)})\s*[\]\)]\s*$"
        quote_set = "\"'‘’“”"
        comments_dict = dict.fromkeys(sectors, "")

        for comment in txt:
            splitter = re.search(pattern, comment)[0]
            if not splitter:
                logger.error(f"Unexpected sector names in text.\n{comment}")
                return None
            else:
                sector = splitter.strip()[1:-1].strip()
                quote = comment.split(splitter)[0].strip()
                if quote[0] in quote_set:
                    quote = quote[1:]
                if quote[-1] in quote_set:
                    quote = quote[:-1]
                if comments_dict[sector] == "":
                    comments_dict[sector] = quote
                else:
                    comments_dict[sector] += " " + quote
    ...
    def _load_report_table(self) -> None:
        ...
        row = self._prep_report_table()

        new_cols = row.keys() - column_map.keys()
        if new_cols:
            raise ValueError(f"No column mapping exists for:\n{new_cols}")

        row = [{column_map[k]: v for k, v in row.items()}]
        with DBConnection() as conn:
            conn.upsert_rows(table_name=table_name, data_rows=row)
    ...
    def _load_rankings_table(self) -> None:
        ...
        row = {"Year": self.year, "Month": self.month} | self.sector_ranking
    ...
    def _prep_report_table(self) -> dict:
        if isinstance(self, ManufacturingPmi):
            ...
            buying_policy_table = self.buying_policy_table[self.buying_policy_table["Month"] == self.month].sort_values(
                "Category"
            )
        ...
        if any(v is None for v in data_row.values()):
